window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			// Per-turn readout: match the built-in usage/time action pills.
			".dsh-ttail{min-width:0;height:calc(28px + var(--dsh-content-font-delta,0px));box-sizing:border-box;color:var(--dsw-alias-label-tertiary);font-size:var(--dsh-content-font-size-secondary,13px);font-variant-numeric:tabular-nums;line-height:calc(24px + var(--dsh-content-font-delta,0px));white-space:nowrap;border:0;border-radius:28px;align-items:center;gap:4px;padding:6px 8px;display:inline-flex;order:999;cursor:pointer;background:transparent;font-family:inherit}",
		...
		const ThinkingTurnTail = react.memo(function ThinkingTurnTail({ messageId, useChat }) {
			...
			return h(Tooltip, { label: detailLabel(stats, "turn"), side: "top" },
				h("button", { type: "button", className: "dsh-ttail", "data-thinking-tail": true, "aria-label": detailLabel(stats, "turn") },
