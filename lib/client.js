window.__ModuleLoader__.load({
	id: "@local/dsh-client-ui-thinking-stats",
	factory: (require) => {
		var module = { exports: {} };
		...
		// The per-turn readout reveals exactly like the timing stats it extends:
		// hidden until the turn-tail row is hovered, so it reads as one unit.
		const CSS_HOVER = [
			"@media (hover:hover){",
			".dsh-ttail{opacity:0;transition:opacity 80ms}",
			"[data-time-hover-root]:hover .dsh-ttail,[data-time-hover-root]:focus-within .dsh-ttail{opacity:1}",
			"}"
		].join("");
		const CSS_TAG_ID = "@local/dsh-client-ui-thinking-stats/styles.css";
		function injectStyles() {
			if (typeof document === "undefined") return;
			if (document.querySelector("style[data-plugin-css=\"" + CSS_TAG_ID + "\"]")) return;
			const tag = document.createElement("style");
			tag.setAttribute("data-plugin", "@local/dsh-client-ui-thinking-stats");
		...
		/** Integer percentage of part over whole, or null when whole is not positive. */
		 ...
		 */
		const ThinkingStatsDock = react.memo(function ThinkingStatsDock({ useSession }) {
		 ...
		 */
		const ThinkingTurnTail = react.memo(function ThinkingTurnTail({ messageId, useSession }) {
			const nodes = useSession((s) => s.chat.legacy.nodes);
