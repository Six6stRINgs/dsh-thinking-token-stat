window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function copy(key) {
			const zh = isChinese();
			return ({
				turnTitle: zh ? "本轮思考 token" : "Thinking tokens this turn",
				sessionTitle: zh ? "本次会话思考 token" : "Thinking tokens (session)",
				all: zh ? "思考 / 全部 token" : "Thinking / all tokens",
				output: zh ? "思考 / 输出 token" : "Thinking / output",
				tokens: zh ? "token" : "tok"
			})[key];
		}
		/** The exact figures shown in the detail panel. */
		function detailLabel(stats, scope) {
			...
			return (
				(scope === "turn" ? copy("turnTitle") + ": " : copy("sessionTitle") + ": ") +
				stats.thinking +
				(totalPct !== null ? " · " + totalPct + (isChinese() ? " 占全部 token" : " of all tokens") : "") +
				(outputPct !== null ? " · " + outputPct + (isChinese() ? " 占输出 token" : " of output tokens") : "")
			);
		}
		//#endregion
		...
		const ThinkingTurnTail = react.memo(function ThinkingTurnTail({ messageId, useChat }) {
			...
			const panel = h("div", { ref: panelRef, className: "dsh-ttail-panel", role: "dialog", "aria-label": detailLabel(stats, "turn"), style: pos || MEASURE_STYLE },
				h("div", { className: "dsh-ttail-title" },
					h("span", { className: "dsh-ttail-title-label" }, copy("turnTitle")),
					h("span", { className: "dsh-ttail-title-value" }, formatTokens(stats.thinking) + " " + copy("tokens"))),
				h("div", { className: "dsh-ttail-rule", "aria-hidden": true }),
				h("dl", { className: "dsh-ttail-details" },
					h("dt", null, copy("all") + " (" + formatTokens(stats.total) + " " + copy("tokens") + ")"), h("dd", null, totalPct || "—"),
					h("dt", null, copy("output") + " (" + formatTokens(stats.output) + " " + copy("tokens") + ")"), h("dd", null, outputPct || "—")));
			return h("span", { ref: rootRef, className: "dsh-ttail-wrap", "data-thinking-tail": true },
				h("button", { type: "button", className: "dsh-ttail", "aria-haspopup": "dialog", "aria-expanded": open, "aria-label": detailLabel(stats, "turn"), onClick: () => setOpen(!open) },
